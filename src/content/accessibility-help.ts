...
export const accessibilityHelpSections: AccessibilityHelpSection[] = [
  ...
  },
  {
    ...
    content:
      "Press Tab to move forward through links, topic cards, the Light and Dark base theme choices, and the High contrast switch. Press Shift+Tab to move backward, then use Enter or Space to activate the focused item. The skip link moves focus directly to the main content.",
  ...
  },
  {
    ...
    content:
      "Choose Light or Dark as the base theme, then turn High contrast on or off independently. High-contrast mode strengthens text, borders, focus outlines, and button contrast without changing the content order. Current navigation state is also shown with text and underline, not color alone.",
